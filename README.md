...
- Соблюдайте формат кода (PEP 8). При необходимости используйте `ruff`/`black` локально (не входят в требования).
- При изменениях в ML-пайплайне обновляйте соответствующие документы и проверяйте совместимость с существующими артефактами.
- Перед публикацией тестируйте команды `/status`, `/retrain`, `/setpolicy` и обработку сообщений в тестовом чате.
- Для профилирования мета-классификатора можно включить уровень логирования `DEBUG`.
...
Задачи отмечаются и синхронизируются через `TODO` и `CLEANUP_REPORT.md`.
